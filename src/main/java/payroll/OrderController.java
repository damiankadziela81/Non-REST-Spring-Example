...
class OrderController {
    ...
    /*
    It checks the Order status before allowing it to be cancelled. If it’s not a valid state, it returns an
    RFC-7807 Problem, a hypermedia-supporting error container. If the transition is indeed valid, it transitions
    ...
    ResponseEntity<?> complete(@PathVariable Long id) {
        ...
        return ResponseEntity
                ...
                .body(Problem.create()
                        .withTitle("Method not allowed")
                        .withDetail("You can't complete order which is in " + order.getStatus() + " status"));
...
It injects both an OrderRepository as well as a OrderModelAssembler.
...
(or a wrapper around such a type).
 */
